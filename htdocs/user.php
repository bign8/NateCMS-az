...
switch ($_REQUEST['action']){
	// Login a user
	case 'login':
		echo $usr->login($_REQUEST['user'], $_REQUEST['pass'], isset($_REQUEST['direct']), $_REQUEST['referer']);
		break;
		
	// Register a user
	case 'register':
		echo $usr->register($_REQUEST['user'], $_REQUEST['pass'], $_REQUEST['email'], $_REQUEST['first'], $_REQUEST['last'], isset($_REQUEST['direct']));
		break;
		
	// Make sure a username doesn't already exist
	case 'check':
		echo $usr->check($_REQUEST['user']); // don't care if direct, 1 or 0
		break;
	
	// Process a forgotten password
	case 'forgot':
		echo $usr->forgot(isset($_REQUEST['direct']));
		break;
	
	// Logout a user
	case 'logout':
		echo $usr->logout(isset($_REQUEST['direct']), $_REQUEST['referer']);
		break;
	
	// Verify a logged in and permissable user
	case 'checkLogged':
		echo $usr->checkLogged();
		break;
